...
public class RelationAnalyzer extends DefaultTraversalVisitor<AnalyzedRelation, RelationAnalysisContext> {
    ...
    protected AnalyzedRelation visitQuerySpecification(QuerySpecification node, RelationAnalysisContext context) {
        ...
        Multimap<String, Symbol> selectList = analyzeSelect(node.getSelect(), !node.getGroupBy().isEmpty(), context);
        List<Symbol> groupBy = analyzeGroupBy(selectList, node.getGroupBy());

        if (node.getSelect().isDistinct() && groupBy.isEmpty()) {
            groupBy = rewriteGlobalDistinct(selectList);
        }
...
//        }
    ...
    private List<Symbol> rewriteGlobalDistinct(Multimap<String, Symbol> selectList) {
        List<Symbol> groupBy = new ArrayList<>(selectList.size());
        for (Symbol symbol : selectList.values()) {
            if (symbol.symbolType() == SymbolType.DYNAMIC_REFERENCE) {
...
//        }
    ...
    private OrderBy analyzeOrderBy(Multimap<String, Symbol> selectList, List<SortItem> orderBy) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            // SortSymbolValidator.validate(symbol); TODO: must be moved to planner because partitionBy columns aren't known here
    ...
    private List<Symbol> analyzeGroupBy(Multimap<String, Symbol> selectList, List<Expression> groupBy) {
        ...
        expressionAnalysisContext.sysExpressionsAllowed = expressionAnalysisContext.sysExpressionsAllowed || !groupBySymbols.isEmpty();
        return groupBySymbols;
    }

    private Multimap<String, Symbol> analyzeSelect(Select select, boolean selectFromFieldCache, RelationAnalysisContext context) {
        SelectAnalyzer.SelectAnalysis selectAnalysis = SelectAnalyzer.newSelectAnalysis(
                context.sources(), expressionAnalyzer, expressionAnalysisContext, selectFromFieldCache);
