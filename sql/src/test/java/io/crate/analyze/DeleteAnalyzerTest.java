...
public class DeleteAnalyzerTest extends BaseAnalyzerTest {
    ...
    public void testDeleteWhere() throws Exception {
        ...
        Function whereClause = (Function)relation.whereClause().query();
    ...
    public void testDeleteWherePartitionedByColumn() throws Exception {
        ...
        assertThat(relation.whereClause().hasQuery(), Matchers.is(false));
        assertThat(relation.whereClause().noMatch(), Matchers.is(false));
        assertEquals(ImmutableList.of(
                        new PartitionName("parted", Arrays.asList(new BytesRef("1395874800000"))).stringValue()),
                relation.whereClause().partitions());

        analysis = analyze("delete from parted");
        relation = analysis.relation();
        assertThat(relation.whereClause().hasQuery(), Matchers.is(false));
        assertThat(relation.whereClause().noMatch(), Matchers.is(false));
        assertEquals(ImmutableList.<String>of(), relation.whereClause().partitions());
    }
